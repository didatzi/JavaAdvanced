...
public class StudentsRepository {
    ...
    private static void readData(String fileName) throws IOException {
        ...
        String regex = "(?<courseName>[A-Z][a-zA-Z#+]*_[A-Z][a-z]{2}_\\d{4})\\s+(?<userName>[A-Z][a-z]{0,3}\\d{2}_\\d{2,4})\\s+(?<studentScore>\\d+)";
        ...
        for (String line : lines) {
            ...
            if (!line.isEmpty() && matcher.find()) {
                ...
                Integer mark = Integer.parseInt(matcher.group("studentScore"));
                if (mark >= 0 && mark <= 100) {
    ...
    public static void printSortedStudents(String course, String comparingType, Integer numberOfStudents){
        if (!isQueryForCoursePossible(course)){
            return;
        }
        if (numberOfStudents == null){
        ...
        RepositorySorters.printSortedStudents(studentsByCourse.get(course), comparingType, numberOfStudents);
    }
}
